...
import { readLastTurns } from '../state/db.js';
/** Build the full system prompt for one agent turn */
export async function buildContext(state, turnHistorySize = 20) {
    const tierConfig = getTierConfig(state.karmaBalance);
    const soul = await readSoul();
    const history = await readLastTurns(turnHistorySize);
    const ttd = estimateTimeToDeathMs(state.karmaBalance, state.totalKarmaSpent / Math.max(state.totalTurns, 1));
