...
export function resolveInferenceConfig(): InferenceConfig | null {
  const provider = (process.env.DEVA_LLM_PROVIDER || 'openai') as InferenceConfig['provider'];
...
export async function openaiCompletion(
  ...
  const toolCalls = Array.isArray(choice?.tool_calls)
    ? (choice.tool_calls as ChatToolCall[])
    : [];
